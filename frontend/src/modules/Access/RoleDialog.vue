  ...
  <v-dialog :model-value="modelValue" max-width="480" @update:model-value="$emit('update:modelValue', $event)">
    <v-card color="#161c2d" border>
      <v-card-title class="dialog-title">
        <v-icon start color="success" size="20">mdi-shield-key-outline</v-icon>
        {{ role ? 'Edytuj rolę' : 'Nowa rola' }}
      </v-card-title>
      <v-divider color="rgba(255,255,255,0.07)" />

      <v-card-text class="pt-5">
        <v-text-field v-model="form.name" label="Nazwa roli *" color="success" class="mb-3" />
        <v-textarea v-model="form.description" label="Opis" rows="3" color="success" />
      </v-card-text>
